...
def PadMask(padded_input, input_lengths):
    ...
    T = shape[1]
    # mask = torch.repeat()
    # mask = torch.where(padded_input == 0, True, False)
    positions = torch.arange(T).expand(N, T)

    # Create mask: True for positions >= sequence_length (padding positions)
    # For each row, compare positions with the corresponding sequence length
