...
class DecoderOnlyTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.dropout                = nn.Dropout(layer_drop_rate) # Dropout
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.enc_layers = nn.ModuleList(
            SelfAttentionEncoderLayer(d_model, num_encoder_heads, d_ff_encoder, dropout=dropout) for _ in range(num_encoder_layers)
        ) # ModuleList of encoder layers

        # TODO: Create decoder layers
        # Use ModuleList to create a list of decoder layers
        self.dec_layers = nn.ModuleList(
            CrossAttentionDecoderLayer(d_model, num_decoder_heads, d_ff_decoder, dropout=dropout) for _ in range(num_decoder_layers)
        )
        # ModuleList of decoder layers

        # TODO: Create source and target embeddings and other layers
        # Use SpeechEmbedding class to create the source embedding
        self.source_embedding = SpeechEmbedding(input_dim, d_model, time_reduction, reduction_method=reduction_method, dropout=dropout) # Speech embedding
