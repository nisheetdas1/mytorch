...
class SequenceGenerator:
    ...
    def generate_greedy(
        ...
        B = x.size(0)
        scores = torch.zeros(B, dtype=torch.float32, device=x.device)
        finished = torch.zeros(B, dtype=torch.bool, device=x.device)

        current_seq_length = 0
        for t in range(self.max_length - current_seq_length):
            a = torch.where(finished == True)[0]
            if len(a) == B:
                break
            ...
            next_token_scores, next_tokens = torch.max(log_probs, dim=-1)

            # Update scores for active sequences
            for i in range(B):
                if not finished[i]:
                    scores[i] += next_token_scores[i]
                    # Mark as finished if EOS token is generated
                    if next_tokens[i] == self.tokenizer.eos_id:
                        finished[i] = True
    ...
    def generate_beam(
        ...
        beam_scores = torch.zeros(B, beam_width, dtype=torch.float, device=self.device)

        # Keep track of finished beams
        finished = torch.zeros(B, beam_width, dtype=torch.bool, device=self.device)

        for step in range(self.max_length - T):
            ...
            if step == 0:
                ...
                beam_scores, tokens = torch.topk(log_probs, beam_width, dim=-1, largest=True, sorted=True)
                tokens = tokens.long()
                ...
                finished = (tokens == self.tokenizer.eos_id)
            ...
            with torch.no_grad():
                for j in range(beam_width):
                    sequences_for_beam = sequences[:, j, :]

                    logits_for_beam = self.score_fn(sequences_for_beam)

                    all_logits_list.append(logits_for_beam.unsqueeze(1))
            ...
            log_probs[:, :, self.tokenizer.eos_id] = torch.where(finished, torch.zeros_like(beam_scores),
                                                                 log_probs[:, :, self.tokenizer.eos_id])

            candidate_scores = beam_scores.unsqueeze(-1) + log_probs

            flat_candidate_scores = candidate_scores.view(B, -1)
            top_scores, top_indices = torch.topk(flat_candidate_scores, beam_width, dim=-1, largest=True,
                                                 sorted=True)

            beam_idx = (top_indices // self.tokenizer.vocab_size).long()
            token_idx = (top_indices % self.tokenizer.vocab_size).long()

            parent_sequences = torch.gather(sequences, 1, beam_idx.unsqueeze(-1).expand(-1, -1, current_len))
            parent_finished = torch.gather(finished, 1, beam_idx)

            sequences = torch.cat([parent_sequences, token_idx.unsqueeze(-1)], dim=-1)
            beam_scores = top_scores
            ended = (token_idx == self.tokenizer.eos_id)
            finished = parent_finished | ended

        final_sequences_list = []
        final_scores_list = []
        for batch_idx in range(B):
            sorted_indices = torch.argsort(beam_scores[batch_idx], descending=True)
            final_sequences_list.append(sequences[batch_idx][sorted_indices])
            final_scores_list.append(beam_scores[batch_idx][sorted_indices])

        sequences = torch.stack(final_sequences_list)
        beam_scores = torch.stack(final_scores_list)
